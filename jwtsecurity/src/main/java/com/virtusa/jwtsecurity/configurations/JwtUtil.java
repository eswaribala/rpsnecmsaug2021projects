...
public class JwtUtil {
	...
	/*
	 * public JwtUtil(VaultConfiguration configuration) { this.vaultConfiguration =
	 * configuration; }
	 */
	...
	public User getUser(final String token) {
		try {
			Claims body = Jwts.parser().setSigningKey("abc123").parseClaimsJws(token).getBody();
			User user = new User();
	...
	public String generateToken(User user) {
		...
		return Jwts.builder().setClaims(claims).setIssuedAt(new Date(nowMillis)).setExpiration(exp)
				.signWith(SignatureAlgorithm.HS512, "abc123").compact();
	}

	public void validateToken(final String token) {
		try {
			Jwts.parser().setSigningKey("abc123").parseClaimsJws(token);
